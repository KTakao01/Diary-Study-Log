...
```markdown
実は、$50 \times 50$ の二次元配列をわざわざ作らなくても、水が届いた座標の文字列（例: "2,2"）を Set （重複を許さないデータ構造）に放り込んでいけば、最後に Set のサイズを取得するだけで済みます。メモリ消費量が劇的に減るテクニックです。
```
...
function main() {
  ...
  for (let i = 0; i < K; i++) {
    ...
    for (let row = -1; row <= 1; row++) {
      for (let col = -1; col <= 1; col++) {
        ...
        // 庭園の範囲内であればSetに追加 (Setなので重複は自動的に無視されます)
        if (1 <= x && x <= N && 1 <= y && y <= M) {
          watered.add(`${x},${y}`);
        }
      }
  ...
  // Setに入っている要素数 = 水が撒かれた区画の総数
  console.log(watered.size);
}
